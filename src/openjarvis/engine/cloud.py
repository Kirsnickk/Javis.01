...
PRICING: Dict[str, tuple[float, float]] = {
    ...
    "qwen/qwen3-32b": (0.39, 0.39),
    "openai/gpt-oss-120b": (0.60, 0.60),
...
def _is_deepseek_model(model: str) -> bool:
    return "deepseek" in model.lower() and not _is_openrouter_model(model)


def _is_mistral_model(model: str) -> bool:
    m = model.lower()
    return (m.startswith("mistral") or m.startswith("codestral") or m.startswith("pixtral") or m.startswith("ministral")) and not _is_openrouter_model(model)


def _is_xai_model(model: str) -> bool:
    return "grok" in model.lower() and not _is_openrouter_model(model)


def _is_groq_model(model: str) -> bool:
    return model in _GROQ_MODELS or model.startswith("groq/") or model.startswith("meta-llama/") or model.startswith("openai/gpt-oss")


def _is_cohere_model(model: str) -> bool:
    return model.lower().startswith("command") and not _is_openrouter_model(model)
...
class CloudEngine(InferenceEngine):
    """Cloud inference via OpenAI, Anthropic, Google, DeepSeek, Mistral, xAI, Cohere, and MiniMax SDKs."""
    ...
    def _init_clients(self) -> None:
        ...
        if mistral_key:
            try:
                ...
                self._mistral_client = openai.OpenAI(
        ...
        if cohere_key:
            try:
                ...
                self._cohere_client = openai.OpenAI(
    ...
    def _get_openai_compat_client(self, model: str) -> Any:
        """Return the OpenAI-compatible client for a given model."""
    ...
    def _generate_openai_compat(
        ...
        **kwargs: Any,
        ...
        """Generate via any OpenAI-compatible provider (DeepSeek, Mistral, xAI, Cohere)."""
        client = self._get_openai_compat_client(model)
        if client is None:
            raise EngineConnectionError(
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if _is_deepseek_model(model) or _is_mistral_model(model) or _is_xai_model(model) or _is_cohere_model(model) or _is_groq_model(model):
            return self._generate_openai_compat(messages, **kw)
        ...
        **kwargs: Any,
        ...
        if _is_codex_model(model):
            ...
            async for token in self._stream_google(messages, **kw):
                yield token
        elif _is_deepseek_model(model) or _is_mistral_model(model) or _is_xai_model(model) or _is_cohere_model(model) or _is_groq_model(model):
            async for token in self._stream_openai_compat(messages, **kw):
                yield token
        else:
    ...
    async def _stream_openai_compat(
        ...
        **kwargs: Any,
        ...
        """Stream via any OpenAI-compatible provider."""
        client = self._get_openai_compat_client(model)
        if client is None:
            raise EngineConnectionError(f"No client for model {model}")
        kwargs.pop("response_format", None)
        create_kwargs: Dict[str, Any] = {
            "model": model,
            "messages": messages_to_dicts(messages),
    ...
    def health(self) -> bool:
        return (
            ...
            or self._cohere_client is not None
    ...
    def close(self) -> None:
        for attr in (
            "_openai_client", "_anthropic_client", "_openrouter_client",
            "_minimax_client", "_deepseek_client", "_mistral_client",
            "_xai_client", "_groq_client", "_cohere_client",
        ):
